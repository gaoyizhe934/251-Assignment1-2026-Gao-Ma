...
You must submit your final work using the stream submission system no later than 18 October
2026 at 11.59 pm. The penalty is 20% deducted from the total possible mark for every day
...
defined below. You will need to use git to manage source code contributions and integration
...
2026-FirstName1-FirstName2.  For  example,  if  the  first  student  is  Sarah  and  the  second
student is Li, then the repository name should be (251-Assignment1-2026-Sarah-Li).

1

After  submitting  your  assignment  on  Stream,  you  must  send  an  invitation  to  your  GitHub
...
●  GitHub Issue Tracker (for logging changes and bugs)
●  Maven (for dependency management and process automation)
●  Docker for packaging and deployment.
●  SpotBugs and PMD (for code quality checks)
●  Other APIs to perform the different functionalities (you are free to search and use any

library that will help in implementing any of the functionalities).
...
a)  use code quality tools to report metrics data of your program. The metrics report
...
a software metrics tool (see some examples above). Code quality report using SonarQube
and PMD should also be submitted with your assignment.

a)  generate a metrics data report from any software metrics tool (see below for the

specific metrics) and add the report file (csv) to $project$/reports/metrics
...
b)  analyse your project for bugs using SpotBugs Maven plugin and add the report files to

$project$/reports/spotbugs.

c)  create a new Maven goal called “pmd” that should generate a metrics report using
...
The README.md is a file with Markdown syntax (this should be correctly formatted
as Markdown) that contains:
...
Correct implementation of the following functions: Save, Search and Print.

1.5

Correct implementation of the PDF conversion functionality

Correct implementation of the following functions: Open (read) .RTF and
.ODT files

Correct implementations of the read source code files  (.java, .py  and .js).
Different syntax should be shown in different colours.

Appropriate use of git FROM THE START OF DEVELOPMENT

Appropriate use of issue tracking to track changes/issues FROM THE
START OF DEVELOPMENT.

Correct use of Maven with all external dependencies correctly added and
the project can compile and run without needing the IDE.
...
Correct deployment into Docker container (upload to load and run correctly)

1.5

Correct use of configuration files (in YAML)

Correct use and implementation of the CI using GitHub Actions

Check code quality and include reports of the size, complexity, and other
quality metrics. Both SpotBugs and PMD are added as a Maven goals
